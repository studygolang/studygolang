...
func (AccountController) Register(ctx *echo.Context) error {
	...
	if username == "" || ctx.Request().Method != "POST" {
		render(ctx, registerTpl, map[string]interface{}{"captchaId": captcha.NewLen(4)})
		return
	}

	// 校验验证码
	if !captcha.VerifyString(ctx.Form("captchaid"), ctx.Form("captchaSolution")) {
		render(ctx, registerTpl, map[string]interface{}{"error": "验证码错误", "captchaId": captcha.NewLen(4)})
		return
	}

	// 入库
	errMsg, err := DefaultUser.CreateUser(ctx, ctx.Request().Form)
	if err != nil {
		// bugfix：http://studygolang.com/topics/255
		if errMsg == "" {
			errMsg = err.Error()
		}
		render(ctx, registerTpl, map[string]interface{}{"error": errMsg})
		return
	}
