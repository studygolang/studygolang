...
export const metadata: Metadata = {
  title: "无回复话题 - Go语言中文网",
  description: "查看所有未收到回复的 Go 语言讨论话题，帮助社区成员解答疑问",
}

async function getNoReplyTopicsData(page: number) {
  const [topicsResult, nodesResult] = await Promise.allSettled([
    fetchAPI<TopicListData>(`/topics/no_reply?p=${page}`, { cache: 'no-store' }),
...
interface NoReplyTopicsPageProps {
  searchParams: Promise<{ p?: string }>
}

export default async function NoReplyTopicsPage({ searchParams }: NoReplyTopicsPageProps) {
  const { p: pageStr } = await searchParams
  const page = Math.max(1, parseInt(pageStr ?? '1', 10) || 1)

  const { topicsData, nodes } = await getNoReplyTopicsData(page)
  ...
  return (
    <PageLayout>
      <PageHeader
        title="无回复话题"
        description="帮助解答社区中尚未收到回复的问题"
        breadcrumbs={[{ label: "主题讨论", href: "/topics" }, { label: "无回复话题" }]}
        actions={
          ...
          <div className="mt-6 flex items-center justify-center gap-2">
            ...
            {end < totalPages && (
              <>
                ...
                <Link
                  href={`/topics/no_reply?p=${totalPages}`}
                  className="rounded-md bg-secondary px-3 py-1.5 text-sm font-medium text-secondary-foreground hover:bg-secondary/80"
                >
                  {totalPages}
                </Link>
