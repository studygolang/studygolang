...
import { OBJTYPE_TOPIC } from "@/lib/types"
import type { TopicDetailData, CommentDetailData, User } from "@/lib/types"
import { formatTime } from "@/lib/utils"

async function getTopicDetail(id: string): Promise<TopicDetailData | null> {
  try {
    return await fetchAPI<TopicDetailData>(
      `/topics/${id}`,
      { cache: 'no-store' }
    )
  } catch {
...
async function getCommentDetail(cid: string, objid: string): Promise<CommentDetailData | null> {
  try {
    return await fetchAPI<CommentDetailData>(
      `/comments/${cid}/detail?objid=${objid}&objtype=${OBJTYPE_TOPIC}`,
      { cache: 'no-store' }
    )
  } catch {
    return null
...
export async function generateMetadata({
  ...
  const [topicData, commentData] = await Promise.all([
    getTopicDetail(id),
    getCommentDetail(cid, id),
  ])

  if (!topicData?.topic || !commentData?.comment) {
    return {
      title: "评论不存在 - Go语言中文网",
    }
  }
  ...
  return {
    title: `第 ${comment.floor} 楼评论 - ${topic.title} - Go语言中文网`,
    description: preview,
  }
}

export default async function TopicCommentPage({
  params,
}: {
  params: Promise<{ id: string; cid: string }>
}) {
    ...
    <PageLayout>
      <PageHeader
        ...
        breadcrumbs={[
          ...
          {
            label: topic.title.length > 20 ? topic.title.slice(0, 20) + "..." : topic.title,
            href: `/topics/${id}`
          },
