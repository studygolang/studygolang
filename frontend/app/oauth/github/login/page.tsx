...
function GithubLoginContent() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/api/v1/oauth/github/url?uri=${encodeURIComponent(redirect)}`, {
      credentials: "include",
    })
      .then((res) => res.json())
      .then((data) => {
        if (data.code === 0 && data.data?.url) {
          ...
          setError(data.msg || "获取 GitHub 授权地址失败")
        }
      })
      .catch(() => {
        setError("网络错误，请稍后重试")
  ...
  return (
    <div className="flex min-h-screen items-center justify-center">
      <div className="text-center">
        ...
        <p className="text-sm text-muted-foreground">正在跳转到 GitHub 登录...</p>
      </div>
    </div>
  )
}
