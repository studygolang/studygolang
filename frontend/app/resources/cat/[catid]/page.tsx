...
export const metadata: Metadata = {
  title: "资源分类 - Go语言中文网",
  description: "按分类浏览 Go 语言学习资源",
}

interface ResourceCatPageProps {
  params: Promise<{ catid: string }>
  searchParams: Promise<{ p?: string }>
}

async function ResourceItems({ page, catid }: { page: number; catid: number }) {
  const qs = `/resources?p=${page}&catid=${catid}`
  ...
  if (!data || !data.resources || data.resources.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center rounded-lg border border-dashed border-border py-16 text-center">
        ...
        <p className="text-sm text-muted-foreground">该分类暂无资源数据</p>
      </div>
    )
  }
  ...
  return (
    <div className="min-w-0 flex-1 space-y-3">
      ...
      <div className="flex items-center justify-center gap-2 pt-4">
        {page > 1 && (
          <Link
            href={`/resources/cat/${catid}?p=${page - 1}`}
...
export default async function ResourceCatPage({ params, searchParams }: ResourceCatPageProps) {
  const { catid: catidStr } = await params
  const { p: pageStr } = await searchParams
  const catid = Math.max(1, parseInt(catidStr, 10) || 1)
